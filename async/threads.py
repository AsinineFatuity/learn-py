...
thread.start()
thread2.start()
# the following .join lines tell our main thread (the one running the program) to wait for these 2 threads to finish before exiting the program
# they are therefore called blocking operations
thread.join()
thread2.join()
